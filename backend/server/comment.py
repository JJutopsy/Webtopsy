...
@comment_bp.route('/comment/email', methods=['POST'])
def post_eml_comment():
    data = request.get_json()
    post_id = data.get('post_id')
    username = data.get('username')
    context = data.get('context')
    ...
    # 새 댓글 추가
    cursor.execute("INSERT INTO emlComments (post_id, username, context, type) VALUES (?, ?, ?, ?)", (post_id, username, context, type))
...
# @comment_bp.route('/comment/email/<int:comment_id>', methods=['DELETE'])
# def delete_eml_comment(comment_id):
#     data = request.get_json()
#     db_path = data.get('db_path')
...
#     # 댓글 삭제
#     cursor.execute("DELETE FROM emlComments WHERE id=?", (comment_id,))
...
def get_eml_comments(post_id):
    ...
    # 댓글 불러오기
    cursor.execute("SELECT * FROM emlComments WHERE post_id=?", (post_id,))
    comments = cursor.fetchall()
    result_list = []
    for row in comments:
        result_list.append({'id': row['id'], 'post_id': row['post_id'], 'username': row['username'], 'context':row['context'], 
...
@comment_bp.route('/recent_comments/email', methods=['POST'])
def get_recent_eml_comments():
    data = request.get_json()
    db_path = data.get('db_path')
    if not os.path.exists(db_path):
    ...
    # 최근 댓글 10개 불러오기
    cursor.execute("SELECT * FROM emlComments ORDER BY created_at DESC LIMIT 10")
    comments = cursor.fetchall()
    result_list = []
    for row in comments:
        result_list.append({'id': row['id'], 'post_id': row['post_id'], 'username': row['username'], 'context':row['context'], 
